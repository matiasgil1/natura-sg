...
import { Search, Printer, Eye, ChevronLeft, ChevronRight, ShoppingBag, Clock } from 'lucide-react';
      ...
      html: `
        <div class="text-left text-[11px] space-y-4">
          ...
          <div class="px-2 space-y-2">
             ...
             <div class="flex justify-between text-green-600 font-black uppercase tracking-widest text-[8px]"><span>Pagado</span><span class="text-sm font-black">${formatCurrency(sale.montoPagado)}</span></div>
             ${sale.saldoRestante > 0 ? `<div class="flex justify-between text-red-500 font-black uppercase tracking-widest text-[8px]"><span>Deuda</span><span class="text-sm font-black">${formatCurrency(sale.saldoRestante)}</span></div>` : ''}
          </div>
        </div>
      `,
                  ...
                  <td className="px-8 py-5">
                    <div className="flex flex-col">
                      ...
                      <span className={`text-[8px] font-black uppercase tracking-widest ${sale.saldoRestante > 0 ? 'text-red-400' : 'text-green-500'}`}>
                        {sale.saldoRestante > 0 ? 'Pendiente' : 'Saldado'}
                      </span>
                    </div>
                  </td>
