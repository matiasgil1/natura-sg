...
import { View, User, Product, Client, Sale } from './types';
...
const App: React.FC = () => {
  ...
  const [data, setData] = useState<{ products: Product[], clients: Client[], sales: Sale[] }>({
    products: [],
    clients: [],
    sales: []
  });
  ...
  const refreshData = async (silent = false) => {
    ...
    try {
      ...
      setData({ products, clients, sales });
      if (silent) Toast.fire({ icon: 'success', title: 'Datos actualizados' });
    } catch (e: any) {
  ...
  if (isLoading && data.products.length === 0 && data.clients.length === 0) {
    return (
  ...
  return (
    <div className="animate-in fade-in duration-1000">
      ...
      >
        {currentView === View.DASHBOARD && <Dashboard sales={data.sales} products={data.products} onRefresh={() => refreshData(true)} />}
        ...
        {currentView === View.CLIENTS && <Clients clients={data.clients} onRefresh={() => refreshData(true)} />}
